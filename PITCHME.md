...
```bash
# Откатим изменения файла или директории
...
# Откатим последний коммит,
# но оставим модифицированные файлы как есть
git reset --soft HEAD^
...
Подробное описание, через пропуск строки,
если необходимо. 
В английской практике удобно читать если описание 
задействует повелительное наклонение -
отвечает на вопрос
...
В случае наличии интеграции с чем-либо,
можно также указать номера stories и issue. E.g., 
...
Вдохновлён SemVer (1.0.0 : MAJOR.MINOR.PATCH) и Angular Commit Guidelines.

Предполагает схлопывание веток с обновлениями и явного обозначения типа изменения:
...
# Используем master, 
# который не мёрджили в нашу отдельную ветку
git rebase -i master
# ...
pick # latest commit
...
@ul
- обозначает законченное привнесение новой функциональности
- позволяет обособить процесс обновления кодовдой базы
- проще настроить автоматизацию и проверки
- облегчает взаимодействие в команде и при opensource разработке
@ulend
...
- разделение ветвей по категориям
- обозначает правила и возможные варианты merge'а
- позволяет конфигурировать часто используемые команды
- доступен в качестве надстройки в GUI и консольных утилитах
...
@ul
- предполагает коллаборацию вокруг одной мастер-ветки
- все feature-ветки недолговечны и сливаются в мастер как можно чаще
- предполагает хорошую культуру PR и code review
