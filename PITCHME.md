...
---?code=src/flask/5-blueprint.py&lang=py&color=white&title=Flask. Widget
@[5](Задействуем модуль Restful)
@[8-9](Некое подобие уникального ключа)
@[12-17](Наша импровизированная БД)
@[20](API и обработка ошибок)
@[22-24](Зададим также обработку аргументомв)
@[27-29](Отобразим сам шаблон)
@[32, 45](Обозначим ресурсы)
@[33](Получение всех записей)
@[36-42](Добавление новой)
@[46-51](Операции с одной конкретной записью)
@[54-55](Добавим соответствующие маршруты)
...
@[29-51](Наш виджет, разметка)
@[58-99](Логика виджета и обработка данных)
@[54-55](Воспользуемся Vue и Axios)
@[59](Адрес нашего приложения)
@[29](Обозначим наш виджет)
@[60-61](Инициализируем Vue)
@[82-88](Получаем список всех записей)
@[73-80](Разделяем их на две группы)
@[39-43](Выводим отдельно, в две колонки)
@[89-92](Добавим новую запись)
@[34-37](Привяжем функциональность к шаблону)
@[5-26](Добавим немного стилей)
